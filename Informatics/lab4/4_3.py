...
def lex_string(string):
    match = re.compile(r"(\"(?:[^\\']|\\['\\/bfnrt]|\\u[0-9a-fA-F]{4})*?\")\s*(.*)", re.DOTALL).match(string)
    if not match:
        return None, string

    json_string, trash = match.groups()
    return eval(json_string), trash


def lex_number(string):
    match = re.compile(r"(-?(?:0|[1-9]\d*)(?:\.\d+)?(?:[eE][+-]?\d+)?)\s*(.*)", re.DOTALL).match(string)
    if not match:
        return None, string

    json_number, trash = match.groups()
    return eval(json_number), trash
...
open("out_dop2.yaml", "w").write(t)
print("Я бы улыбнулся, но нету ведь повода...")
