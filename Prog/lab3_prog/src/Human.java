public abstract class Human implements Abilities{
    ...
    @Override
    public void shooting(PlasticGun gun, int times){
        if(times > gun.getCountOfBullets())
            System.out.println("Ты не можешь выстрелить больше раз, чем пуль в твоем пистолете!");
        else{
            for(int i = 0; i < times; ++i){
                System.out.printf("Бах! Произошел выстрел... В пистолете осталость %d пуль\n", (gun.getCountOfBullets() - times));
            }
        }
    }
    @Override
